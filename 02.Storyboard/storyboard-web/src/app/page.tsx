...
import { Plus, Film, Trash2, Search, SlidersHorizontal, Clock, Grid3X3, Loader2 } from "lucide-react";
...
export default function Home() {
  ...
  const handleCreate = async (data: {
    ...
    const result = await createProject({
      ...
      creativeGoal: data.creativeGoal || undefined,
      targetAudience: data.targetAudience || undefined,
      videoTone: data.videoTone || undefined,
    });
    setCreating(false);
    if (result.success) {
      setShowCreate(false);
                  ...
                  {filtered.map((project) => (
                    ...
                    >
                      ...
                      <p className="text-xs text-text-muted">
                        {project.shotCount} 个镜头 · {project.imageCount} 张图片 · {formatUpdatedAt(project.updatedAt)}
                      </p>
            ...
            {selected ? (
              <div className="p-4 space-y-4">
                ...
                <div className="space-y-2 text-xs text-text-secondary">
                  <p>画幅：{selected.aspectRatio || "未设置"}</p>
                  ...
                  <p>更新：{formatUpdatedAt(selected.updatedAt)}</p>
                </div>
                <button
